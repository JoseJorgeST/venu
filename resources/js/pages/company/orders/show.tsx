...
export default function OrderShow({ order, statuses }: Props) {
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: `Pedido #${order.id}`, href: `/company/orders/${order.id}` },
    ];

    const handleStatusChange = (newStatus: string) => {
        router.patch(`/company/orders/${order.id}/status`, {
            status: newStatus,
        }, {
            preserveScroll: true,
        });
            ...
            <div className="flex h-full flex-1 flex-col gap-6 p-6">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <Button variant="outline" size="icon" onClick={() => router.visit('/company/orders')}>
